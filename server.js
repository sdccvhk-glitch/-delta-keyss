...
function adminAuth(req, res, next) {
  ...
  const expectedPass = String(process.env.ADMIN_PASSWORD || "change-this-password");
  const expectedSecret = String(process.env.ADMIN_SECRET || "change-this-secret");
  const suppliedSecret = String(req.headers["x-admin-secret"] || "");
  if (String(user || "").trim().toLowerCase() !== expectedEmail || String(pass || "") !== expectedPass || String(suppliedSecret || "") !== expectedSecret) {
    return res.status(401).json({ error: "Invalid admin login or secret code." });
  }
  next();
}
...
app.post("/api/orders", (req, res) => {
  ...
  if (!utr) return res.status(400).json({ error: "UTR / UPI transaction ID is required." });
  if (utr.length < 6 || utr.length > 100) return res.status(400).json({ error: "Please enter a valid UTR / transaction ID." });
