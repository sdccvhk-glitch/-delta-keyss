...
// Step 1: create an unpaid order. Customers must start the UPI payment flow
// before the server will accept a UTR for that order. Manual UPI still requires
// the admin to verify the real transaction before releasing a key.
app.post("/api/payments/start-order", async (req, res) => {
  try {
    ...
    const order = { id: localId, productId: product.id, productName: product.name, planId: plan.id, duration: plan.name, amount: Number(plan.price), paymentReference: "", status: "awaiting_payment", key: "", paymentStartedAt: now, createdAt: now, updatedAt: now };
    ...
    if (!upiId) return res.status(503).json({ error: "UPI ID is not configured. Ask the admin to add the receiving UPI ID in Settings." });
    const amount = Number(plan.price).toFixed(2);
    const upiLink = `upi://pay?pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent(payeeName)}&am=${encodeURIComponent(amount)}&cu=INR&tn=${encodeURIComponent("DELTA.KEYS " + localId)}`;
    const qrDataUrl = await QRCode.toDataURL(upiLink, { errorCorrectionLevel: "M", margin: 2, width: 360 });
    res.json({ order: publicOrder(order), upiLink, qrDataUrl, upiId, payeeName, amount, message: "Order created. Complete the UPI payment before submitting your UTR." });
  } catch (e) {
    console.error(e);
...
// Step 2: attach the UTR to an existing payment-started order.
app.post("/api/payments/submit-utr", (req, res) => {
  try {
    const { orderId, paymentReference } = req.body || {};
    const reference = String(paymentReference || "").trim();
    if (!orderId || !reference) return res.status(400).json({ error: "Order ID and UPI transaction ID / UTR are required." });
    ...
    if (order.status !== "awaiting_payment") return res.status(409).json({ error: "This order is no longer accepting a UTR." });
    if (!order.paymentStartedAt) return res.status(400).json({ error: "Start the UPI payment first." });
    const duplicate = store.orders.some(o => o.id !== order.id && o.paymentReference && String(o.paymentReference).toLowerCase() === reference.toLowerCase() && o.status !== "rejected");
