...
async function api(url,opt={}){
  ...
  opt.headers={...(opt.headers||{}),Authorization:auth,'X-Admin-Secret':secret,'Content-Type':'application/json'};
...
async function loadAdmin(){
  ...
  $('orders').innerHTML=orders.length?orders.map(o=>`<article class="order-card"><div class="order-top"><div><h3>${esc(o.productName)}</h3><div class="order-meta">Order: ${esc(o.id)}<br>Amount: ₹${Number(o.amount).toFixed(0)}<br>UTR: ${esc(o.paymentReference||'—')}<br>Payment: manual verification required</div></div><span class="status ${o.status}">${o.status}</span></div><div class="admin-controls"><input id="key-${o.id}" value="${escAttr(o.key||'')}" placeholder="Edit customer key"><select id="status-${o.id}"><option ${o.status==='pending'?'selected':''}>pending</option><option ${o.status==='approved'?'selected':''}>approved</option><option ${o.status==='rejected'?'selected':''}>rejected</option></select><button onclick="saveOrder('${o.id}')">Save / Approve</button></div></article>`).join(''):`<div class='glass form-card'><p class='muted'>No orders yet.</p></div>`;
}
