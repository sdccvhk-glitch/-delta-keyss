...
function setupNavDnD(){
  ...
  navButtons().forEach(btn=>{
    btn.addEventListener('click',e=>{
      if(dragSuppressClick){dragSuppressClick=false;e.preventDefault();e.stopPropagation();return;}
      openPage(btn.dataset.page);
    });
    btn.addEventListener('pointerdown',e=>{
      if(e.pointerType==='mouse' && e.button!==0)return;
      dragCandidate=btn;dragPointerId=e.pointerId;dragMoved=false;dragStartX=e.clientX;dragStartY=e.clientY;
      try{btn.setPointerCapture(e.pointerId)}catch{}
    });
    btn.addEventListener('pointermove',e=>{
      if(!dragCandidate||e.pointerId!==dragPointerId)return;
      const dx=e.clientX-dragStartX,dy=e.clientY-dragStartY;
      if(!dragMoved && Math.hypot(dx,dy)>6){
        dragMoved=true;draggingNav=dragCandidate;draggingNav.classList.add('dragging');
      }
      if(draggingNav&&dragMoved){e.preventDefault();moveNavItem(e.clientX,e.clientY)}
    });
    btn.addEventListener('pointerup',e=>{
      if(e.pointerId!==dragPointerId)return;
      e.preventDefault();
      try{btn.releasePointerCapture(e.pointerId)}catch{}
      if(draggingNav)finishNavDrag();
      else {dragCandidate=null;dragPointerId=null;}
    });
    btn.addEventListener('pointercancel',e=>{
      if(e.pointerId===dragPointerId){try{btn.releasePointerCapture(e.pointerId)}catch{};if(draggingNav)draggingNav.classList.remove('dragging');draggingNav=null;dragCandidate=null;dragPointerId=null;}
    });
    // Desktop native drag fallback.
    btn.addEventListener('dragstart',e=>{
      draggingNav=btn;dragMoved=true;btn.classList.add('dragging');
      e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',btn.dataset.page);
    });
    btn.addEventListener('dragover',e=>{e.preventDefault();if(draggingNav&&draggingNav!==btn)moveNavItem(e.clientX,e.clientY)});
    btn.addEventListener('dragend',()=>{if(draggingNav)finishNavDrag()});
  });
