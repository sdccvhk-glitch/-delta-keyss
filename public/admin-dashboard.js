...
async function loadAdmin(){
  ...
  $('stats').innerHTML=`<div class="admin-stat"><strong>${orders.length}</strong><span>Total orders</span></div><div class="admin-stat"><strong>${orders.filter(o=>o.status==='pending').length}</strong><span>Awaiting approval</span></div><div class="admin-stat"><strong>${orders.filter(o=>o.status==='paid').length}</strong><span>Paid orders</span></div><div class="admin-stat"><strong>${out.keyPoolCount||0}</strong><span>Vault keys</span></div>`;
  ...
  $('orders').innerHTML=orders.length?orders.map(o=>`<article class="order-card"><div class="order-top"><div><h3>${esc(o.productName)}</h3><div class="order-meta">Order: ${esc(o.id)}<br>Duration: ${esc(o.duration||'—')}<br>Amount: ₹${Number(o.amount).toFixed(0)}<br>UPI UTR: <strong>${esc(o.paymentReference||'—')}</strong></div></div><span class="status ${o.status}">${o.status}</span></div><div class="admin-controls"><select id="status-${o.id}"><option ${o.status==='pending'?'selected':''}>pending</option><option ${o.status==='paid'?'selected':''}>paid</option><option ${o.status==='rejected'?'selected':''}>rejected</option></select><button onclick="saveOrder('${o.id}')">${o.status==='pending'?'Verify & Approve':'Update order'}</button></div></article>`).join(''):`<div class='glass form-card'><p class='muted'>No orders yet.</p></div>`;
  refreshUploadPlans();
}
