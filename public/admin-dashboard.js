...
let draggingNav=null,dragMoved=false,dragPointerId=null,dragTimer=null;
function setupNavDnD(){
  const nav=$('adminNav'); if(!nav)return;
  navButtons().forEach(btn=>{
    btn.addEventListener('click',()=>{if(btn.dataset.skipClick==='1'){btn.dataset.skipClick='0';return}openPage(btn.dataset.page)});
    btn.addEventListener('dragstart',e=>{draggingNav=btn;btn.classList.add('dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',btn.dataset.page)});
    btn.addEventListener('dragover',e=>{e.preventDefault();if(!draggingNav||draggingNav===btn)return;const r=btn.getBoundingClientRect();if(e.clientY<r.top+r.height/2)nav.insertBefore(draggingNav,btn);else nav.insertBefore(draggingNav,btn.nextSibling)});
    btn.addEventListener('dragend',async()=>{if(draggingNav){draggingNav.classList.remove('dragging');draggingNav=null;await saveNavOrder()}});
    btn.addEventListener('pointerdown',e=>{
      if(e.pointerType==='mouse'&&e.button!==0)return;
      dragMoved=false;dragPointerId=e.pointerId;
      clearTimeout(dragTimer);
      dragTimer=setTimeout(()=>{draggingNav=btn;btn.classList.add('dragging');try{btn.setPointerCapture(e.pointerId)}catch{}},180);
    });
    btn.addEventListener('pointermove',e=>{
      if(dragPointerId!==e.pointerId||!draggingNav)return;
      dragMoved=true;const target=document.elementFromPoint(e.clientX,e.clientY)?.closest('.nav-item');
      if(!target||target===draggingNav||!nav.contains(target))return;
      const r=target.getBoundingClientRect();
      if(e.clientY<r.top+r.height/2)nav.insertBefore(draggingNav,target);else nav.insertBefore(draggingNav,target.nextSibling);
      e.preventDefault();
    });
    btn.addEventListener('pointerup',async e=>{
      ...
      dragPointerId=null;
      if(draggingNav){draggingNav.classList.remove('dragging');draggingNav=null;if(dragMoved){btn.dataset.skipClick='1';await saveNavOrder()}}
    });
    btn.addEventListener('pointercancel',()=>{clearTimeout(dragTimer);dragPointerId=null;if(draggingNav){draggingNav.classList.remove('dragging');draggingNav=null}});
  });
  const saved=JSON.parse(localStorage.getItem('delta:adminNavOrder')||'null');
  if(Array.isArray(saved))applyNavOrder(saved);
